...
- The repository has no existing `v1.0.0` tag or release. The local GitHub API check returns HTTP 404 and `git ls-remote` returns exit code 2 for the absent tag.
...
4. Query the remote tag and classify only exit code 2 as "not found".
