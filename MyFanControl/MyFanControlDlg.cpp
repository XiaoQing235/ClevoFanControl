...
CMyFanControlDlg::CMyFanControlDlg(CWnd* pParent)
...
BEGIN_MESSAGE_MAP(CMyFanControlDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_AUTO_SWITCH, &CMyFanControlDlg::OnBnClickedAutoSwitch)
	ON_BN_CLICKED(IDC_BUTTON_MANAGE_PRESETS, &CMyFanControlDlg::OnBnClickedManagePresets)
	ON_BN_CLICKED(IDC_BUTTON_CURVE_ADD, &CMyFanControlDlg::OnBnClickedCurveAdd)
	ON_BN_CLICKED(IDC_BUTTON_CURVE_DELETE, &CMyFanControlDlg::OnBnClickedCurveDelete)
	ON_BN_CLICKED(IDC_BUTTON_CURVE_RESET, &CMyFanControlDlg::OnBnClickedCurveReset)
	ON_CBN_SELCHANGE(IDC_COMBO_CURVE, &CMyFanControlDlg::OnCbnSelchangeCurve)
	ON_CBN_SELCHANGE(IDC_COMBO_CLOSE_BEHAVIOR, &CMyFanControlDlg::OnCbnSelchangeCloseBehavior)
	ON_CONTROL(EN_CHANGE, IDC_EDIT_INTERVAL, &CMyFanControlDlg::OnConfigurationEditChanged)
	ON_CONTROL(EN_CHANGE, IDC_EDIT_TREANSITION, &CMyFanControlDlg::OnConfigurationEditChanged)
	ON_CONTROL(EN_CHANGE, IDC_EDIT_FORCE_TEMP, &CMyFanControlDlg::OnConfigurationEditChanged)
	ON_CONTROL(EN_CHANGE, IDC_EDIT_FONT_SIZE, &CMyFanControlDlg::OnFontSizeChanged)
	ON_CONTROL(EN_KILLFOCUS, IDC_EDIT_NODE_INDEX, &CMyFanControlDlg::OnNodeEditKillFocus)
	ON_CONTROL(EN_KILLFOCUS, IDC_EDIT_NODE_TEMP, &CMyFanControlDlg::OnNodeEditKillFocus)
	ON_CONTROL(EN_KILLFOCUS, IDC_EDIT_NODE_DUTY, &CMyFanControlDlg::OnNodeEditKillFocus)
	ON_MESSAGE(WM_FAN_CURVE_CHANGED, &CMyFanControlDlg::OnFanCurveChanged)
...
BOOL CMyFanControlDlg::OnInitDialog()
{
	...
	m_hCoreThread = reinterpret_cast<HANDLE>(_beginthreadex(
		NULL, 0, &CMyFanControlDlg::CoreThread, this, 0, &threadId));
...
unsigned __stdcall CMyFanControlDlg::CoreThread(void* lParam)
{
	CMyFanControlDlg* dialog = reinterpret_cast<CMyFanControlDlg*>(lParam);
	if (dialog != NULL)
	{
		dialog->m_core.Run();
	}
...
void CMyFanControlDlg::OnGetMinMaxInfo(MINMAXINFO* lpMMI)
{
...
void CMyFanControlDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CMyFanControlDlg::LayoutControls(int cx, int cy)
{
...
void CMyFanControlDlg::MoveControl(int id, int left, int top, int width, int height, BOOL show)
{
...
void CMyFanControlDlg::InitializeStatusColumns()
{
...
void CMyFanControlDlg::ResizeStatusColumns(int listWidth)
{
...
void CMyFanControlDlg::UpdateDpi()
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd))
	{
		return;
...
void CMyFanControlDlg::ApplyUIFont()
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd))
	{
		return;
...
int CMyFanControlDlg::MeasureUiTextWidth(LPCTSTR text) const
{
	if (text == NULL || m_hWnd == NULL || !::IsWindow(m_hWnd))
	{
		return 0;
	}
	CClientDC dc(const_cast<CMyFanControlDlg*>(this));
	CFont* oldFont = NULL;
...
int CMyFanControlDlg::GetUiTextHeight() const
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd))
	{
		return 0;
...
int CMyFanControlDlg::ScaleX(int value) const
{
	return std::max(1, MulDiv(value, m_dpiX, 96));
}

int CMyFanControlDlg::ScaleY(int value) const
{
	return std::max(1, MulDiv(value, m_dpiY, 96));
}

void CMyFanControlDlg::SetInitialWindowSize()
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd))
	{
		return;
...
void CMyFanControlDlg::RefreshStatus()
{
	if (m_ctlStatus.GetSafeHwnd() == NULL)
	{
		return;
...
void CMyFanControlDlg::StopCoreThread()
{
	if (m_hCoreThread != NULL)
	{
...
std::string CMyFanControlDlg::ConfigurationPath() const
{
...
std::string CMyFanControlDlg::PresetConfigurationPath() const
{
	CStringA path(GetExePath());
	path += "\\";
...
BOOL CMyFanControlDlg::SavePresetCollection(const PresetCollection& collection, CString* error)
{
...
BOOL CMyFanControlDlg::ApplyPresetAt(int index, BOOL showError, BOOL automatic)
{
...
BOOL CMyFanControlDlg::ApplyGlobalConfiguration(BOOL showError)
{
	FanConfig candidate = m_globalConfig;
	std::string validationError;
	if (!candidate.Validate(&validationError))
...
void CMyFanControlDlg::ScanPresetProcesses()
{
...
FanCurvePoints* CMyFanControlDlg::SelectedCurve()
{
	return m_nSelectedCurve == 0 ? &m_draft.CpuCurve : &m_draft.GpuCurve;
}

const FanCurvePoints* CMyFanControlDlg::SelectedCurve() const
{
	return m_nSelectedCurve == 0 ? &m_draft.CpuCurve : &m_draft.GpuCurve;
}

void CMyFanControlDlg::LoadDraftFromCore()
{
...
BOOL CMyFanControlDlg::LoadDraftFromFile(BOOL showWarning)
{
	FanConfig loaded;
	std::string diagnostic;
...
BOOL CMyFanControlDlg::LoadPresetCollection(BOOL showWarning)
{
...
void CMyFanControlDlg::SetEditInteger(CEdit& edit, int value)
{
	CString text;
...
BOOL CMyFanControlDlg::ParseEditInteger(const CEdit& edit, int* value) const
{
...
void CMyFanControlDlg::SyncSelectedNodeToControls()
{
...
void CMyFanControlDlg::RefreshCurves()
{
...
void CMyFanControlDlg::SyncDraftToControls()
{
...
void CMyFanControlDlg::SyncPresetControls()
{
	m_bSyncingControls = TRUE;
...
void CMyFanControlDlg::UpdateActivePresetStatus()
{
...
void CMyFanControlDlg::OnFontSizeChanged()
{
...
void CMyFanControlDlg::SetDraftDirty(BOOL dirty)
{
	m_bDraftDirty = dirty;
	SetWindowText((m_bDraftDirty || m_bPresetStoreDirty) ?
		_T("MyFanControl *") : _T("MyFanControl"));
}

BOOL CMyFanControlDlg::ApplyNodeEditor(BOOL showError)
{
	FanCurvePoints* curve = SelectedCurve();
	if (curve == NULL || curve->empty())
	{
...
BOOL CMyFanControlDlg::ReadDraftFromControls()
{
	int value = 0;
...
BOOL CMyFanControlDlg::ValidateDraft(CString* error) const
{
	if (error != NULL)
	{
		error->Empty();
...
BOOL CMyFanControlDlg::SaveDraft()
{
...
void CMyFanControlDlg::OnBnClickedCurveAdd()
{
...
void CMyFanControlDlg::OnBnClickedCurveDelete()
{
	ApplyNodeEditor(FALSE);
	FanCurvePoints* curve = SelectedCurve();
...
void CMyFanControlDlg::OnBnClickedCurveReset()
{
	ApplyNodeEditor(FALSE);
...
void CMyFanControlDlg::OnCbnSelchangeCurve()
{
	ApplyNodeEditor(FALSE);
...
void CMyFanControlDlg::OnNodeEditKillFocus()
{
	ApplyNodeEditor(FALSE);
}

LRESULT CMyFanControlDlg::OnFanCurveChanged(WPARAM wParam, LPARAM lParam)
{
...
void CMyFanControlDlg::OnBnClickedAutoSwitch()
{
	if (m_bSyncingControls)
	{
		return;
...
void CMyFanControlDlg::OnBnClickedManagePresets()
{
...
void CMyFanControlDlg::OnCbnSelchangeCloseBehavior()
{
	m_draft.CloseToTray = m_closeBehavior.GetCurSel() == 1;
	SetDraftDirty(TRUE);
}

void CMyFanControlDlg::OnConfigurationEditChanged()
{
	if (m_bSyncingControls)
	{
		return;
...
BOOL CMyFanControlDlg::SaveBeforeClose()
{
	...
	const int choice = MessageBox(_T("Save the current configuration before closing?"),
		_T("MyFanControl"), MB_YESNOCANCEL | MB_ICONQUESTION);
	if (choice == IDCANCEL)
	{
		return FALSE;
	}
...
void CMyFanControlDlg::ShowWindowFromTray()
{
...
void CMyFanControlDlg::HideWindowToTray()
{
	if (m_bShuttingDown || m_hWnd == NULL || !::IsWindow(m_hWnd))
	{
		return;
...
void CMyFanControlDlg::SetTray(PCSTR string)
{
...
LRESULT CMyFanControlDlg::OnTaskbarCreated(WPARAM, LPARAM)
{
	m_bTrayAdded = FALSE;
	SetTray("MyFanControl");
	return 0;
...
BOOL CMyFanControlDlg::SetAutorunReg(BOOL, BOOL)
{
	return FALSE;
}
...
{
	const CString taskName = _T("MyFanControl");
	const CString targetPath = GetExePath() + _T("\\MyFanControl.exe");
	const CString xmlPath = GetExePath() + _T("\\MyFanControl.task.xml");
	if (!bWrite)
	{
		CString output = ExecuteCmd(_T("SCHTASKS /Query /TN \"MyFanControl\""));
...
CString CMyFanControlDlg::ExecuteCmd(CString command)
{
